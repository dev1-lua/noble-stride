...
export class ListGreylistedInvestorsTool implements LuaTool {
  ...
  description =
    "STAFF-ONLY. List investors currently classified Greylisted (optionally also Excluded). Internal read-only reference: returns each investor's name, classification, type, and CRM deep link, never raw record ids. Use when staff ask which investors are greylisted or excluded.";
  inputSchema = inputSchema;

  constructor(private deps?: { crm: CrmClient; isStaff?: StaffCheck }) {}

  async execute(input: z.infer<typeof inputSchema>) {
    // Investor classification is internal staff data — self-authorize first, before any CRM access.
    ...
    // Keep in lockstep with the write-guard classification set (see investor-tracker guards).
    const wanted = new Set<string>(input.includeExcluded ? ["Greylisted", "Excluded"] : ["Greylisted"]);
