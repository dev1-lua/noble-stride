...
| **Deals** | `/deals` | The **unified deals queue** (spec §3.2 "Deal/Mandate"), consolidating mandates and transactions into one list — Jira-style table by default (search/sort/filter/group/CSV export/saved views), with the former Kanban boards available behind a List/Board toggle. Mandate rows are the sell-side **engagement contracts** (one per client fundraising assignment: deal size, sector, NDA/EA contract dates, "the assignment NobleStride was hired for"); transaction rows are the **live deals** executed under a mandate (target raise, instrument, deal type, stage, milestones, VDR link, success fee — the record investors get matched against). A **Type** column distinguishes the two, and each row opens its own detail page (`/mandates/[id]` or `/transactions/[id]`). The old `/mandates` and `/transactions` list routes redirect here (`/deals?type=mandate`, `/deals?type=transaction`). |
