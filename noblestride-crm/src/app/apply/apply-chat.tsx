...
// Hosts LuaPop for the Website Intake & Qualification Agent (SOW §10) inside
// an isolated iframe (srcdoc) — same pattern (and same accepted-risk note) as
// src/app/talk-to-us/talk-to-us-chat.tsx. A fresh sessionId is generated per
// mount so shared/public computers never resume a stranger's conversation;
// "New chat" just regenerates it (remounting the iframe).
  ...
  const config = {
    ...
    embeddedDisplayConfig: {
      ...
      conversationStarters: [
        "We're raising capital — I'd like to apply",
        "What information do you need from us?",
        "We're an existing applicant with an update",
      ],
    },
    ...
    welcomeMessage:
      "Welcome to Noblestride Capital. I'll take your funding application step by step — a few questions about your company, your raise, and how to reach you — and pass it straight to our deal team for review. Shall we start with your company's legal name?",
  };
  return `<!doctype html>
<html>
<head>
...
export function ApplyChat({ agentId, channelId = "" }: { agentId: string; channelId?: string }) {
      ...
      <div className="min-h-0 flex-1 overflow-hidden rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-primary)]">
        {sessionId && (
          <iframe
            ...
            title="Apply to Noblestride"
            className="h-full w-full border-0"
            allow="microphone; clipboard-write"
            // ACCEPTED RISK (2026-07-14 review, inherited from talk-to-us-chat):
            // no sandbox attribute, so this srcdoc iframe shares the CRM origin
            // with the third-party LuaPop UMD bundle (supply-chain XSS
            // exposure). sandbox without allow-same-origin breaks the widget;
            // sandbox with it is a no-op. Mitigation plan: host this embed on a
            // separate origin (e.g. chat.noblestride.com) before production
            // launch.
          />
        )}
      </div>
