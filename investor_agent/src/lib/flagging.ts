...
/**
 * Pure dedupe core: record a flag only when this sender has NO event inside the
 * trailing window. Keeps one flag per sender per window (spec §4.4 — "deduped to once").
 */
...
/**
 * Records ONE deterministic security flag event for `sender`, keyed by sender, in the
 * "security_flags" collection — deduped to once per trailing window. Returns whether a
 * new event was written. FAIL-OPEN: any Data API error resolves to false and never throws,
 * so flagging can never block or break real investor mail.
 */
