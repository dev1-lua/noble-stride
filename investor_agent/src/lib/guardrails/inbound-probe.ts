...
// Targeted patterns for the external threat surface. Deliberately narrow to keep
// false positives off benign investor mail — this is a non-blocking flag backstop,
// not a gate, so precision matters more than recall.
...
/**
 * Pure, deterministic classification of inbound email text. Never performs I/O and never
 * blocks — it labels a message so the preprocessor can raise a deterministic flag while
 * still letting the sender receive a warm, safe reply (spec §4.2, D5).
 */
